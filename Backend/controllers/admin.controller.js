...
export const getAllUsers = async (req, res) => {
  try {
    ...
    let query = User.find({})
      .select("-password -__v -updatedAt -enrolledCourses")
  ...
  try {
    ...
    if (isLiveActiveCourse) {
      ...
      enrollments.forEach((e, i) => {
        if (!e.userId) {
          console.log("BROKEN USER FOUND →", i, e._id, e.userId);
        }
      });
  ...
  try {
    const coursesWithCounts = await Course.aggregate([
      ...
      },
      {
        $addFields: {
          studentCount: {
            $ifNull: [{ $arrayElemAt: ["$enrollments.count", 0] }, 0],
          },
        },
      },
      {
...
export const getCourseDetailsFull = async (req, res) => {
  try {
    ...
    // 2. Sections + Lectures
    const courseWithSections = await Course.findById(courseId)
      .populate({
        path: "sections",
        populate: {
          path: "lectures", // make sure your schema supports this
        },
      })
      .lean();

    const sections = courseWithSections.sections || [];

    // 3. Students (reuse your logic)
    const enrollments = await CourseEnrollment.find({ courseId })
...
export const getAllCoupons = async (_, res) => {
  try {
    const coupons = await Coupon.find({}).populate("itemId");
